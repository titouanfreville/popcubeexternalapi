...
function build_images() {
  # First args is tags
  docker build --no-cache -t ${REPO}/externalapidb:$1 -f docker/database.Dockerfile .
  docker build --no-cache -t ${REPO}/externalapi:$1 -f docker/gobase.Dockerfile .
}
function push_images() {
  # First args is tags
  docker push ${REPO}/externalapidb:$1
  docker push ${REPO}/externalapi:$1
}
...
OUTPUT=`curl -X POST --header 'Content-Type: application/json' \
  ...
  "{
    ...
    \"Hostname\": \"popcube_ex_alpha_database\" }" \
 ...
 "{
   ...
   \"Env\": [
     ...
     \"VIRTUAL_PORT=3000\",
      ...
      \"MYSQL_USER=test_user\",
      \"MYSQL_DATABASE=popcube_dev\",
     \"VIRTUAL_HOST=ex-api-alpha.popcube.xyz\",
     \"LETSENCRYPT_HOST=ex-api-alpha.popcube.xyz\",
     \"LETSENCRYPT_EMAIL=contact@popcube.xyz\"
   ],
    \"HostConfig\": {
      \"Links\": [
                  \"/popcube_ex_alpha_database:/popcube_ex_alpha_api/database\"
              ]
    },
   \"Hostname\": \"popcube_ex_alpha_api\" }" \
    http://${DEPLOY_URL}/deploy`
if [[ ${OUTPUT} == *"Failed"* ]]; then
echo "${OUTPUT}"
exit 1
